...
@maintenance_bp.route('/maintenance/types/add', methods=['GET', 'POST'])
@login_required
def add_maintenance_type():
    ...
    return render_template(
        'maintenance/type_form.html',
        form=form,
        title='Add Maintenance Type')


@maintenance_bp.route('/maintenance/types/<int:id>/edit',
                      methods=['GET', 'POST'])
@login_required
...
def aircraft_maintenance(aircraft_id):
    ...
    return render_template(
        'maintenance/aircraft_maintenance.html',
        aircraft=aircraft,
        records=records)


@maintenance_bp.route('/aircraft/<int:aircraft_id>/maintenance/add',
                      methods=['GET', 'POST'])
@login_required
def add_maintenance_record(aircraft_id):
    ...
    form.maintenance_type.choices = [(t.id, t.name)
                                     for t in MaintenanceType.query.all()]
    form.performed_by.choices = [
        (u.id, u.full_name) for u in User.query.filter_by(
            is_admin=True).all()]
    if form.validate_on_submit():
        ...
        return redirect(
            url_for(
                'maintenance.aircraft_maintenance',
                aircraft_id=aircraft_id))
    return render_template(
        'maintenance/record_form.html',
        form=form,
        aircraft=aircraft,
        title='Add Maintenance Record')


@maintenance_bp.route('/aircraft/<int:aircraft_id>/squawks')
@login_required
def aircraft_squawks(aircraft_id):
    aircraft = Aircraft.query.get_or_404(aircraft_id)
    squawks = Squawk.query.filter_by(
        aircraft_id=aircraft_id).order_by(
        Squawk.created_at.desc()).all()
    return render_template(
        'maintenance/aircraft_squawks.html',
        aircraft=aircraft,
        squawks=squawks)


@maintenance_bp.route('/aircraft/<int:aircraft_id>/squawks/add',
                      methods=['GET', 'POST'])
@login_required
def add_squawk(aircraft_id):
    ...
    if form.validate_on_submit():
        ...
        return redirect(
            url_for(
                'maintenance.aircraft_squawks',
                aircraft_id=aircraft_id))
    return render_template(
        'maintenance/squawk_form.html',
        form=form,
        aircraft=aircraft,
        title='Add Squawk')


@maintenance_bp.route('/squawks/<int:id>/edit', methods=['GET', 'POST'])
@login_required
def edit_squawk(id):
    ...
    if form.validate_on_submit():
        ...
        return redirect(
            url_for(
                'maintenance.aircraft_squawks',
                aircraft_id=squawk.aircraft_id))
    return render_template(
        'maintenance/squawk_form.html',
        form=form,
        aircraft=squawk.aircraft,
        title='Edit Squawk')
